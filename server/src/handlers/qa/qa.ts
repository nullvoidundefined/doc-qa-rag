...
export async function streamQA(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    // 5. Send citations
    res.write(`data: ${JSON.stringify({ type: "citations", citations: chunks })}\n\n`);

    if (chunks.length === 0) {
      const noContextMsg = "I don't have any documents to search through. Please upload some documents first.";
      res.write(`data: ${JSON.stringify({ type: "token", token: noContextMsg })}\n\n`);

      const assistantMsg = await convRepo.createMessage(conversationId, "assistant", noContextMsg);
      res.write(`data: ${JSON.stringify({ type: "done", conversation_id: conversationId, message_id: assistantMsg.id })}\n\n`);
      res.end();
      return;
    }
