...
export async function uploadDocument(req: Request, res: Response): Promise<void> {
  ...
  if (!allowedMimes.includes(file.mimetype)) {
    res.status(400).json({ error: { message: "Unsupported file type. Upload PDF, TXT, or MD files." } });
    return;
  }

  const maxSize = 10 * 1024 * 1024; // 10MB
  if (file.size > maxSize) {
    res.status(400).json({ error: { message: "File too large. Maximum size is 10MB." } });
    return;
  }
  ...
  logger.info(
    { event: "document_uploaded", documentId: document.id, userId: user.id, filename: file.originalname },
...
export async function deleteDocument(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    logger.warn({ err, r2Key: document.r2_key }, "Failed to delete file from R2");
  }
